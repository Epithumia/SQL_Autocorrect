...
from sql_autocorrect.models import blank_ast
from sql_autocorrect.models.statut import (
    ...
    MauvaisAgregat,
    MauvaiseColAgregat,
    MauvaisDistinctAgregat,
)

class ExceptionParseur(Exception):
...
#             key=lambda x: (
#                 x is not None,
#                 "" if isinstance(x, Number) else type(x).__name__,
#                 x,
#             ),
#         )
#         rs2 = sorted(
...
#                 statut.append(AliasManquant(w, f"{w} {item['value']}", m))
#                 m = 0
#     return correct, statut
...
#                 if "select_distinct" in sql:
#                     sql["select"] = sql["select_distinct"]
...
#                 solutions["select"] = list(
#                     map(list, unique_everseen(map(tuple, solutions["select"]), str))
#                 )
#                 sol_from = sql["from"]
...
#                 solutions["from"] = list(
#                     map(list, unique_everseen(map(tuple, solutions["from"])))
#                 )
...
#                     solutions["orderby"] = list(
#                         map(
#                             list, unique_everseen(map(tuple, solutions["orderby"]), str)
#                         )
#                     )
...
#                     solutions["groupby"] = list(
#                         map(
#                             list, unique_everseen(map(tuple, solutions["groupby"]), str)
#                         )
#                     )
...
#                     solutions["where"] = list(
#                         map(list, unique_everseen(map(tuple, solutions["where"]), str))
#                     )
...
#                     solutions["having"] = list(
#                         map(list, unique_everseen(map(tuple, solutions["having"]), str))
#                     )
#     return solutions
...
#             isinstance(token["value"], dict) and ag in token["value"].keys()
#             for ag in agregats
#         ):
...
#         return correct, statut
#     if (
#         all(("having" in sol.keys() and len(sol["having"])) for sol in sols)
#         and "having" not in sql.keys()
#     ):
#         min_having = 9999
#         for sol in sols:
#             s = sol["having"]
...
#         return correct, statut
#     if (
#         not any("having" in sol.keys() and len(sol["having"]) for sol in sols)
#         and "having" in sql.keys()
#     ):
#         correct = False
...
#     if "all_columns" in sql_select[0] and "*" not in solutions["select"]:
#         correct = False
...
#     manque = 9999
#     if (
#         isinstance(sql_select[0], dict)
#         and "value" in sql_select[0].keys()
#         and isinstance(sql_select[0]["value"], dict)
#         and "distinct" in sql_select[0]["value"].keys()
#     ):
...
#             if (
#                 desordre
#                 and len(sol) == len(prop)
#                 and all(sol[i] == prop[i] for i in range(len(sol)))
#             ):
#                 desordre = False
#         if desordre:
#             correct = False
...
def parse_column(col):
    ...
    if "ExprWithAlias" in col:
        column["alias"] = col["ExprWithAlias"]["alias"]["value"]
        expr = col["ExprWithAlias"]["expr"]
    elif "UnnamedExpr" in col:
        expr = col["UnnamedExpr"]
    elif "Wildcard" in col:
        column["is_wildcard"] = True
    elif "QualifiedWildcard" in col:
        column["is_wildcard"] = True
        column["name"] = col["QualifiedWildcard"][0][0]["value"]
    else:
        print(col)
        raise ExceptionParseur("Column: " + str(col))
    if expr is not None:
        if "Identifier" in expr:
            column["name"] = expr["Identifier"]["value"]
        elif "Function" in expr:
            func = expr["Function"]
            if func["name"][0]["value"] in ["COUNT", "MAX", "MIN", "AVG", "SUM"]:
                column["aggregate"] = {
                    "type": func["name"][0]["value"],
                    "args": func["args"],
            ...
            blank[0]["Query"]["body"]["Select"]["projection"] = [col]
            column["expression"] = restore_ast(blank)[0][7:]
        else:
            raise ExceptionParseur("Expression: " + str(expr))
    return column
...
def parse_solutions(fichier):
    ...
    with open(fichier, "r") as f:
        ...
        for sol in parsed:
            solution = {
                "DISTINCT": True
                if sol["Query"]["body"]["Select"]["distinct"]
                else False
            }
            query = sol["Query"]["body"]["Select"]
...
def parse_args(argv):
    ...
    parser.add_argument(
        "-f", type=str, required=True, help="Fichier à analyser", metavar="FICHIER"
    )
    parser.add_argument(
        "-s", type=str, required=True, help="Fichier de solution(s)", metavar="FICHIER"
    )
    parser.add_argument(
        "-r",
        type=str,
        required=True,
        default="resultat.sqlac",
        help="Fichier dans lequel stocker le résultat (par défaut : res.sqlac)",
        metavar="FICHIER",
    )
    parser.add_argument(
        "-db",
        type=str,
        required=True,
        help="Base sur laquelle exécuter la requête",
        metavar="BDD",
    )
